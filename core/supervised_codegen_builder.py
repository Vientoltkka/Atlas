...
- En las pruebas focalizadas, PROHIBIDO hardcodear valores derivables mecanicamente de la entrada (len, conteos, sumas, ordenaciones, frecuencias, etc.): calculalos programaticamente sobre la propia fixture y afirmalos contra el resultado, por ejemplo assert resultado["total"] == len(lineas) o assert ordenado == sorted(lineas). Un numero escrito a mano por el modelo (conteo mental) no es aceptable: el host ejecuta esas pruebas y fallaran ante cualquier error de conteo. Los ejemplos entrada -> salida pueden fijar comportamiento estructural, pero todo valor derivado debe ser matematicamente verificable.
